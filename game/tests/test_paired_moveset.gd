...
## The full paired moveset -- the moves that survived the cut, their
## trajectories, and the seeded pools that let a wrestler reach more than
## one move per tier.
##
## It was 17 moves (11 grapple + 6 reversal). The power, finisher and
## reversal families were removed for not reading on screen, leaving three
## grapples and two signatures. These tests are what keeps the .tres
## library and the recipe file from drifting apart in either direction:
## after that cut, a stale trajectory nobody could reach any more would
## have been invisible without them.
##
