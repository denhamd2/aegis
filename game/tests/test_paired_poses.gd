...
func test_generated_track_paths_resolve_against_a_real_wrestler() -> void:
	...
	var anim := PAIRED_POSES.get_animation("grapple_clinch_knee__attacker")
...
func test_roles_resolve_to_different_clips() -> void:
	assert_str(PairedRecipes.role_clip(&"grapple_clinch_knee", true)) \
		.is_not_equal(PairedRecipes.role_clip(&"grapple_clinch_knee", false))
...
func test_a_wrestler_can_play_its_half_of_an_authored_move() -> void:
	...
	move.animation_pair_id = &"grapple_clinch_knee"
